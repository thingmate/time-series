export class GrowableBuffer {
  ...
  grow(extraLength: number): number {
    const currentLength: number = this.#length;
    const newLength: number = currentLength + extraLength;
  ...
  shrink(length: number): void {
    if (length > this.#length) {
      throw new Error('Shrinking length is greater than current length.');
    }
    this.#length = length;
  }
}
