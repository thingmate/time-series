...
export abstract class TimeSeries<GValue> implements AsyncDisposable {
  abstract insert(entries: readonly TimeSeriesEntry<GValue>[]): void;

  push(time: number, value: GValue): void {
    this.insert([{ time, value }]);
  }

  abstract select(options?: TimeSeriesSelectOptions): TimeSeriesEntry<GValue>[];
